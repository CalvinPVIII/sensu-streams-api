...
class EpisodeMasterClass {
  ...
  async returnMovie(series, movieNumber) {
    try {
      let movie = await await this.getEpisode(this.movies[series][movieNumber]);
      return movie;
    } catch (error) {
      ...
      episode.dub.sources.map(async (source) => {
        if (!this.currentNonWorkingSources.includes(source.source)) {
          ...
          if (source.source === "Gogoanime") {
            const gogoFiles = await this.gogoAnimeScrape(source.video);
            if (gogoFiles != "error") {
              const obj = {
                source: "Gogoanime",
                files: gogoFiles,
          ...
          if (source.source === "Gogo") {
            const gogoFiles = await this.gogoPlayScrape(source.video);
            if (gogoFiles != "error") {
              const obj = {
                source: "Gogo",
                files: gogoFiles,
  ...
  async getAllDuration(episodeObject) {
    for (const episode in episodeObject) {
      ...
      let subLength = await this.getDuration(episodeObject[episode].sub.video);
  ...
  async initializeEpisode() {
    ...
    let subEpisodeDuration =
      this.streamPlaylist[this.streamStatus.currentEpisode].sub.episodeLength;

    let currentDubSources =
      this.streamPlaylist[this.streamStatus.currentEpisode].dub.sources;
    let dubEpisodeDuration =
      this.streamPlaylist[this.streamStatus.currentEpisode].dub.episodeLength;
  ...
  async handleVideoStream() {
    ...
    if (this.streamStatus.isActive) {
      ...
      if (this.streamStatus.currentTime >= this.streamStatus.episodeDuration) {
        this.handleMoveToNextEpisode();
        console.log(`current episode is ${this.streamStatus.currentEpisode}`);
      }
